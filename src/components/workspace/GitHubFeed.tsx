...
export default function GitHubFeed({ repoUrl, isFullHeight = false }: { repoUrl: string | null, isFullHeight?: boolean }) {
  ...
  useEffect(() => {
    ...
    async function fetchCommits() {
      ...
      setError(null);
      try {
        const response = await fetch(`https://api.github.com/repos/${repoUrl}/commits?per_page=10`);
  ...
  return (
    <div className="glass-panel" style={{ 
      ...
      maxHeight: isFullHeight ? "none" : "250px",
      overflowY: "auto",
      flexShrink: 0,
      display: "flex",
      ...
      {!repoUrl && (
        <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", textAlign: "center", padding: "var(--spacing-xl)", color: "var(--color-text-muted)", fontSize: "0.9rem" }}>
          No GitHub repository linked. Link one in the header to see recent commits!
        </div>
      )}
